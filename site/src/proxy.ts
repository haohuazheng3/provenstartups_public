...
export const config = {
  // Dot-containing asset paths stay excluded, but .well-known is a routed
  // request. If Clerk's proxy is skipped there, the shared not-found layout
  // calls auth() without middleware context and logs a server error for an
  // otherwise ordinary browser probe.
  matcher: ["/((?!_next|.*\\..*).*)", "/api/(.*)", "/__clerk/(.*)", "/.well-known/(.*)"],
};
