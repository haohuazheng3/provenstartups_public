...
const SEO_OVERRIDES: Record<string, { title: string; description: string }> = {
  "ugc-tank-invite-only-creator-sourcing-platform": {
    title: "UGC Tank Revenue: $598 MRR, Model & Risks",
    description:
      "How UGC Tank reached $598 MRR from two brand subscriptions in 24 hours — and why retention, disintermediation, and founder-led sales remain the real risks.",
  },
};
...
export async function generateMetadata({
  ...
  return {
    title: override?.title ?? `${p.name} — ${p.revenue ?? "Proven AI Startup Idea"}`,
      ...
      {memberWall ? (
        ...
        <>
          ...
          {isGuest && (
            <section className="mt-6">
              <SignupCta
                note="The quick-reference card — acquisition channels, replication playbook, and risk map — unlocks with a free account."
                redirectTo={`/projects/${p.slug}`}
              />
            </section>
          )}
          ...
          <section className="mt-6">
            ...
            <div className="mt-4 space-y-4">
              ...
              {isGuest && (
                <SignupCta
                  note="Source video links unlock with a free account. Full transcripts are Pro."
                  redirectTo={`/projects/${p.slug}`}
                />
              )}
