...
/**
 * 给所有开启通知的账号排队一封新项目提醒 —— 免费账号也收。
 * 2026-09-22 起免费账号唯一的权益就是这封邮件;之前只发会员,而 8 个注册者注册后无一回访。
 */
export async function enqueueNewProjectAlert(items: { name: string; slug: string; tagline: string }[]) {
  ...
  const members = await db
    ...
    .from(users)
    .where(and(eq(users.notifyNewProjects, true), sql`${users.email} not like '%@provenstartups.com'`));
...
<p style="margin:28px 0 0;color:#a8a29e;font-size:12px">You're getting this because new-idea alerts are on for your account. <a href="${SITE.url}/account" style="color:#a8a29e">Turn them off</a>.</p>
</div>`;
