...
export const projects = pgTable(
  "projects",
  {
    ...
    // ── 筛选模块用的归一化字段(2026-09-22,由 scripts/backfill-filters.ts 从文本字段推导)──
    // revenue 原文是 "$23K/mo · 600 users" 这种自由文本,筛"月收入区间"必须有一个数字列。
    // 换算规则见回填脚本;算不出来的留 null,筛选时按"未披露"处理,不冒充 0。
    revenueMonthlyUsd: integer("revenue_monthly_usd"),
    // solo | duo | team | company | unknown —— 从 team 文本归一
    teamSize: text("team_size"),
    // 拆解正文里点名的工具/平台(受控词表),给"用的工具"筛选用
    tools: text("tools").array(),
    // ISO-3166 两位国家码,从 region 推导;无法判断留 null
...
/**
 * 会员的构建记录:配额(每周 SITE.buildsPerWeek、每月 SITE.buildsPerMonthCap)按 clerk_user_id 数行;
 * 结果按 (project_slug, inputs_hash) 缓存 —— 同样的项目+同样的输入不再调用模型、不再计次。
 */
export const builds = pgTable(
  "builds",
  {
    ...
    inputs: jsonb("inputs").$type<{ stack?: string; budget?: string; hoursPerWeek?: string; notes?: string }>(),
    output: text("output"),
    model: text("model"),
  ...
  (t) => [
    index("builds_user_idx").on(t.clerkUserId, t.createdAt),
    index("builds_cache_idx").on(t.projectSlug, t.inputsHash),
  ]
);
